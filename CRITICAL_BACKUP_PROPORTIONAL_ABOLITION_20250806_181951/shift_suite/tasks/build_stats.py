...
def build_stats(
    ...
    *,
    ...
    # 動的スロット設定: app.pyからのslot_minutes（分）を時間に変換
    slot_hours = slot_minutes / 60.0
    log.info(f"[build_stats] 動的スロット設定: {slot_minutes}分 = {slot_hours}時間")
    ...
    if len(heat_all_df.index) > 1:
        try:
            ...
            if len(valid_times) > 1:
                ...
                detected_slot_hours = (time2_dt - time1_dt).total_seconds() / 3600
                if abs(detected_slot_hours - slot_hours) > 0.01:  # 1分以上の差がある場合
                    log.warning(f"[build_stats] 設定値({slot_hours}h)と検出値({detected_slot_hours}h)に差異があります")
            else:
                log.info(f"[build_stats] 有効時間帯インデックス2未満、設定値{slot_hours}h使用")
        except Exception as e_slot:
            log.info(f"[build_stats] スロット幅自動検出失敗({e_slot})、設定値{slot_hours}h使用")
    ...
    try:
        ...
        lack_total = int(
            round(
                # EMERGENCY_FIX: 水増しされた代表値を集計から除外
                overall_df.loc[
