...
class LightweightAnomalyDetector:
    ...
    def __init__(self, sensitivity: str = "medium"):
        ...
        # SLOT_HOURS計算精度保護強化
        self._verify_slot_hours_calculation_integrity()
    ...
    def _verify_slot_hours_calculation_integrity(self):
        """SLOT_HOURS計算精度保護・検証機能"""
        try:
            # SLOT_HOURS値の検証
            expected_slot_hours = 0.5
            if abs(SLOT_HOURS - expected_slot_hours) > 0.001:
                log.error(f"[AnomalyDetector] SLOT_HOURS値異常: {SLOT_HOURS} != {expected_slot_hours}")
                raise ValueError(f"SLOT_HOURS計算値異常: {SLOT_HOURS}")
            
            # 計算精度テスト
            test_slots = 10
            expected_hours = test_slots * SLOT_HOURS
            calculated_hours = self._convert_slots_to_hours(test_slots)
            
            if abs(calculated_hours - expected_hours) > 0.001:
                log.error(f"[AnomalyDetector] スロット時間変換異常: {calculated_hours} != {expected_hours}")
                raise ValueError(f"スロット時間変換計算異常")
            
            log.info(f"[AnomalyDetector] SLOT_HOURS計算精度検証完了 (値: {SLOT_HOURS})")
            
        except Exception as e:
            log.error(f"[AnomalyDetector] SLOT_HOURS計算精度検証失敗: {e}")
            raise
    
    def _convert_slots_to_hours(self, slot_count: int) -> float:
        """スロット数を時間に変換（計算精度保護付き）"""
        if not isinstance(slot_count, (int, float)) or slot_count < 0:
            raise ValueError(f"無効なスロット数: {slot_count}")
        
        # SLOT_HOURS計算保護
        hours = slot_count * SLOT_HOURS
        
        # 計算結果検証
        if hours < 0 or hours > 24 * 7:  # 1週間の時間上限チェック
            log.warning(f"[AnomalyDetector] 計算結果異常値: {hours}時間")
        
        return hours
    
    def _validate_calculation_accuracy(self, calculated_value: float, context: str) -> bool:
        """計算精度検証機能"""
        try:
            # 基本検証
            if not isinstance(calculated_value, (int, float)):
                log.error(f"[AnomalyDetector] 計算結果型異常 ({context}): {type(calculated_value)}")
                return False
            
            if calculated_value < 0:
                log.error(f"[AnomalyDetector] 計算結果負値異常 ({context}): {calculated_value}")
                return False
            
            # コンテキスト固有検証
            if "hours" in context.lower() and calculated_value > 24 * 30:  # 月間時間上限
                log.warning(f"[AnomalyDetector] 時間計算値高値警告 ({context}): {calculated_value}")
            
            return True
            
        except Exception as e:
            log.error(f"[AnomalyDetector] 計算精度検証エラー ({context}): {e}")
            return False
