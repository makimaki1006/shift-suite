    ...
    if show_integrated_dashboard and ComprehensiveDashboard is not None and CURRENT_SCENARIO_DIR is not None:
        try:
            ...
            comprehensive_dashboard_content = [
                ...
                # サマリー統計カード
                html.Div([
                    html.H4("📊 高度分析指標", style={'color': '#1f2937', 'marginBottom': '15px'}),
                ...
                # 統合ダッシュボード図表
                html.Div([
                    html.H4("📈 統合分析ダッシュボード", style={'color': '#1f2937', 'marginBottom': '15px'}),
                    dcc.Graph(
    ...
    try:
        ...
        if not df_shortage_role.empty:
            ...
            try:
                ...
                content.append(html.P(f"職種別グラフ作成エラー: {str(e)}", style={'color': '#dc2626', 'fontWeight': '600'}))
        ...
        events_df = data_get('shortage_events')
        ...
        return html.Div([
            html.H3("不足分析", style={'color': '#dc2626', 'fontWeight': '600'}),
            html.P(f"エラーが発生しました: {str(e)}", style={'color': '#dc2626', 'fontWeight': '600'}),
            html.P(f"エラータイプ: {type(e).__name__}", style={'color': '#dc2626', 'fontWeight': '600'}),
            html.P("詳細はログを確認してください。", style={'color': '#dc2626', 'fontWeight': '600'})
        ])
...
def create_proportional_abolition_tab(selected_scenario: str = None) -> html.Div:
    ...
    try:
        ...
        # DataFrameが None の場合は空のDataFrameを作成
        if df_proportional_role is None:
            df_proportional_role = pd.DataFrame()
        if df_proportional_org is None:
            df_proportional_org = pd.DataFrame()
        ...
        if df_proportional_role.empty:
            ...
            content.append(html.Div([
                html.H4("⚠️ データ不足", style={'color': '#f59e0b', 'fontWeight': '600'}),
                html.P([
                    "按分廃止分析結果が見つかりません。",
                    html.Br(),
        ...
        if not df_proportional_role.empty:
            ...
            content.append(dash_table.DataTable(
                ...
                style_table={'backgroundColor': '#ffffff', 'marginBottom': '30px'},
                style_cell={
                    'textAlign': 'center',
                    'padding': '12px',
            ...
            content.append(html.Div([
                ...
                html.Div([
                    ...
                    html.Div([
                        html.H5("⚪ 従来の按分方式の問題:", style={'color': '#374151'}),
            ...
            if len(shortage_roles) > 0:
                ...
                content.append(html.Ol(action_items, style={'fontSize': '16px', 'lineHeight': '1.6'}))
        ...
        return html.Div([
            html.H3("🎯 按分廃止・職種別分析"),
            html.P(f"エラーが発生しました: {str(e)}", style={'color': '#dc2626', 'fontWeight': '600'}),
            html.P("データが正しく読み込まれているか確認してください。", style={'color': '#374151'})
        ])
                    ...
                    if leave_csv_file.exists():
                        ...
                        try:
                            ...
                            if not found_alternative:
                                content.append(html.P("❌ 全てのシナリオで leave_analysis.csv が見つかりませんでした。", style={'color': '#dc2626', 'fontWeight': '600'}))
...
app.layout = html.Div([
    ...
    html.Div([
        html.Div([
            html.H4("🎯 分析シナリオ選択", style={'color': '#1f2937', 'marginBottom': '10px'}),
    ...
    html.Div([
        html.Div([
            html.H4("📊 システム状態", style={'color': '#1f2937', 'marginBottom': '10px'}),
...
def initialize_fact_book_analysis_content(style, selected_scenario, data_status):
    ...
    try:
        if FACT_BOOK_INTEGRATION_AVAILABLE:
            ...
            return html.Div([
                html.H3("📊 統合ファクトブック分析", style={'color': '#1f2937'}),
...
def create_facts_analysis_display(results: dict) -> html.Div:
    ...
    if not objective_facts:
        ...
        return html.Div([
            html.H4("📋 客観的事実分析結果"),
            html.Div([
                html.P(f"⚠️ {error_msg}", style={'color': 'orange', 'textAlign': 'center', 'padding': '24px'})
...
def create_unified_constraints_content(analysis_results):
    ...
    if soft_constraints:
        constraint_sections.append(
            html.Div([
                html.H6("🔧 SOFT制約（推奨）", style={'color': '#d97706', 'fontWeight': '600', 'marginBottom': '10px'}),
    ...
    def execute_truth_discovery_analysis(n_clicks, selected_scenario):
        ...
        try:
            ...
            results_div = html.Div([
                # サマリー
                html.Div([
                    html.H4("🎯 真実あぶり出し分析結果", style={'color': '#1f2937', 'marginBottom': '15px'}),
                    html.Div([
                        html.Div([
    ...
    try:
        if FACT_BOOK_INTEGRATION_AVAILABLE:
            ...
            result_layout = html.Div([
                html.H4("📊 統合ファクトブック分析結果", style={'color': '#1f2937'}),
