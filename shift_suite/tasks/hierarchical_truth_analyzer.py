    ...
    def _analyze_current_staffing(self, data: DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            for col in time_columns:
                # 休暇除外フィルター適用
                # 配置分析用: 休暇データ保持、動的スロット対応
                filtered_data = apply_rest_exclusion_filter(data, f"staffing_analysis_{col}", for_display=False, exclude_leave_records=False)
                staff_count = filtered_data[col].notna().sum()
                staffing_analysis["time_distribution"][str(col)] = staff_count
            
            # 職種別配置分析
            if 'role' in data.columns:
                # 職種別分析: 休暇レコード保持、精密分析用
                filtered_data = apply_rest_exclusion_filter(data, "role_analysis", for_display=False, exclude_leave_records=False)
            ...
            if 'employment' in data.columns:
                # 雇用形態別分析: 休暇レコード保持、統計精度重視
                filtered_data = apply_rest_exclusion_filter(data, "employment_analysis", for_display=False, exclude_leave_records=False)
...
class ProportionalFallbackEngine:
    ...
    def _proportional_shortage_analysis(self, dataset: QualityAssuredDataset, result: TruthAnalysisResult) -> TruthAnalysisResult:
        ...
        try:
            data = dataset.data
            # 按分分析: 休暇データ保持、動的スロット対応、精密計算用
            filtered_data = apply_rest_exclusion_filter(data, "proportional_analysis", for_display=False, exclude_leave_records=False)
