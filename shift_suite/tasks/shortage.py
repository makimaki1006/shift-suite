"""
shortage.py – v2.8.0 (直接修正版)
────────────────────────────────────────────────────────
* v2.8.0: 古い曜日パターン計算ロジックを完全に削除し、
          need_per_date_slot.parquet の利用を強制することで問題を解決。
"""
...
def shortage_and_brief(
    ...
    *,
    ...
    log.info("[shortage] v2.8.0 (直接修正版) 処理開始")

    # --- 1. 必須ファイルの読み込み ---
    try:
        heat_all_df = pd.read_parquet(out_dir_path / "heat_ALL.parquet")
    except FileNotFoundError:
        log.error("[shortage] 致命的エラー: heat_ALL.parquet が見つかりません。処理を中断します。")
        return None
    except Exception as e:
        log.error(f"[shortage] heat_ALL.parquet の読み込みエラー: {e}", exc_info=True)
        return None

    # --- 2. [最重要] 正しいNeedデータを読み込む ---
    # 古いロジックを削除し、このファイルの利用を強制する
    try:
        log.info("★★★ [shortage] 正しいNeedデータ (need_per_date_slot.parquet) の読み込みを試みます...")
        need_df_all = pd.read_parquet(out_dir_path / "need_per_date_slot.parquet")
        need_per_date_slot_df = need_df_all.copy()
        log.info("★★★ ...読み込み成功。これをマスターNeedとして使用します。")
    except FileNotFoundError:
        log.error("!!!!!!!!!! [shortage] 致命的エラー: need_per_date_slot.parquet が見つかりません。 !!!!!!!!!!")
        log.error("!!!!!!!!!! [shortage] このファイルがないと正しい不足計算ができません。heatmap.pyの実行を確認してください。")
        return None  # 処理を完全に中断
    except Exception as e:
        log.error(f"[shortage] need_per_date_slot.parquet の読み込み中に予期せぬエラー: {e}", exc_info=True)
        return None

    # --- 3. 実績データとNeedデータの整形 ---
    # 双方に存在する日付列のみを対象とし、データの整合性を担保する
    common_date_columns = [col for col in heat_all_df.columns if col in need_df_all.columns and col not in SUMMARY5]
    date_columns_in_heat_all = common_date_columns
    if not common_date_columns:
        log.error("[shortage] 致命的エラー: 実績データとNeedデータで共通の日付列が見つかりません。")
        return None

    staff_actual_data_all_df = heat_all_df[common_date_columns].copy().reindex(index=time_labels).fillna(0)
    # need_df_allも実績データと全く同じ形に整形する
    need_df_all = need_df_all[common_date_columns].copy().reindex(index=time_labels).fillna(0)
    log.info(f"[shortage] {len(common_date_columns)}日分の実績とNeedデータを照合・整形しました。")
    ...
    if meta_fp.exists():
        try:
            ...
            estimated_holidays_set.update(
                {
                    d
                    for d in (_parse_as_date(h) for h in meta.get("estimated_holidays", []))
                    if d
                }
            )
            log.info(f"[SHORTAGE_DEBUG] heatmap.meta.json から読み込んだ休業日数: {len(estimated_holidays_set)}")
        except Exception as e_meta:
            log.warning(f"[shortage] heatmap.meta.json 解析エラー: {e_meta}")

    # --- 4. 全体の不足を計算 ---
    log.info("[shortage] 修正されたマスターNeedに基づき、全体の不足時間を計算します。")
    lack_count_overall_df = (need_df_all - staff_actual_data_all_df).clip(lower=0).fillna(0)
    shortage_ratio_df = (
    ...
    log.info("[shortage] ... shortage_time.parquet の保存完了。")
