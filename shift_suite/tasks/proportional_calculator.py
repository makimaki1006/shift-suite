...
class ProportionalCalculator:
    ...
    def __init__(self, slot_minutes: int = 30):
        ...
        slot_minutes : int, default 30
            スロット間隔（分）
        """
        self.slot_hours = validate_and_convert_slot_minutes(slot_minutes, "ProportionalCalculator.__init__")
        self.slot_minutes = slot_minutes
        self.tolerance = 0.01  # 1分未満の誤差は許容
        
        log.info(f"[ProportionalCalculator] 初期化完了: スロット{slot_minutes}分={self.slot_hours}時間")
    ...
    def calculate_total_shortage_from_data(
        ...
        # 修正: 動的スロット設定対応 + 安全な計算
        total_shortage_hours = safe_slot_calculation(
            shortage_by_slot, 
            self.slot_minutes, 
            operation="sum", 
            function_name="calculate_total_shortage_hours"
        )
