...
def build_skill_matrix(long_df: pd.DataFrame, out_dir: Path):
    if "staff" not in long_df.columns or "code" not in long_df.columns:
        log.error("[skill_nmf] long_dfに 'staff' または 'code' 列が見つかりません。処理をスキップします。")
        empty_skill_df = pd.DataFrame(columns=["skill_score"])
        save_df_xlsx(empty_skill_df, out_dir / "skill_matrix.xlsx", sheet_name="skill")
        log.info("skill_nmf: 'staff'または'code'列がなかったため、空のskill_matrix.xlsxを作成しました。")
        return empty_skill_df # または None

    # スタッフ識別列を "name" から "staff" に変更
    mat = (
        long_df.groupby(["staff", "code"]).size().unstack(fill_value=0)
    )

    if mat.empty or mat.shape[0] == 0 or mat.shape[1] == 0:
        log.warning("[skill_nmf] NMFの入力となる行列が空または次元が0です。スキルスコアは計算されません。")
        empty_skill_df = pd.DataFrame(columns=["skill_score"])
        save_df_xlsx(empty_skill_df, out_dir / "skill_matrix.xlsx", sheet_name="skill")
    ...
    if n_components < 1: # NMFが実行できない場合
        log.warning(f"[skill_nmf] NMFのn_componentsが1未満 ({n_components}) のため実行できません。スキルスコアは計算されません。")
        skill = pd.Series(index=mat.index, name="skill_score", dtype=float).fillna(0.0) # 全て0など
    else:
        try:
            model = NMF(
            ...
            skill_scores = W[:, 0] if W.shape[1] > 0 else pd.Series(0.0, index=mat.index) # Wが空でないかチェック
            ...
            log.error(f"[skill_nmf] NMFの計算中にエラーが発生しました: {e}。スキルスコアは0として記録します。")
