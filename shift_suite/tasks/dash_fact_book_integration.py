...
def register_fact_book_callbacks(app):
    """
    ファクトブック分析のコールバックをDashアプリに登録
    ...
    try:
        ...
        # ファクトブック分析コールバックの取得
        callback_func = create_fact_book_analysis_callback()
        
        # コールバックの登録（実際のコールバック定義はdash_app.py側で行う）
        log.info("[FACT_BOOK] コールバック登録準備完了")
        
        return callback_func
        
    except Exception as e:
        log.error(f"[FACT_BOOK] コールバック登録エラー: {e}")
        return None
