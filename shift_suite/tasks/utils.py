...
import tempfile
import zipfile
from datetime import datetime, timedelta # ★ dt エイリアスではなく datetime, timedelta を直接使用
...
# ────────────────── 2. Excel 日付ユーティリティ ──────────────────
def excel_date(excel_serial) -> dt.date | None: # ★ datetime → dt.date に変更 (日付のみ返すため)
...
def save_df_xlsx(
    ...
    *,
    ...
    final_sheet_name = sheet_name or safe_sheet(fp_path.stem) # ★ 変数名を final_sheet_name に変更
...
def write_meta(target: Path | str, /, **meta) -> Path:
    ...
    try:
        meta_fp.write_text(
            json.dumps(meta, ensure_ascii=False, indent=2, default=str), # ★ default=str を追加 (datetime等非シリアライズ可能オブジェクト対策)
            encoding="utf-8",
        )
...
def derive_min_staff(heat: DataFrame | Series, method: str = "p25") -> Series:
    ...
    if isinstance(heat, Series):
        ...
        if method == "mode":
            ...
            return modes_df.iloc[:, 0].round() if not modes_df.empty else pd.Series(index=values.index, dtype=float).fillna(np.nan)
...
def calculate_jain_index(values: pd.Series) -> float:
    ...
    if numeric_values.empty or len(numeric_values) == 0: # ★ len(numeric_values) == 0 もチェック
        return 1.0  # Consider what to return for empty or all-NaN series
    if (numeric_values < 0).any(): # Jain index is typically for non-negative resource allocation
        log.warning("Jain index calculation: input series contains negative values. Results may be misleading.")
    ...
    if sum_of_squares == 0: # Avoid division by zero if all values are zero
        return 1.0 if numeric_values.nunique() <=1 else 0.0 # All zero is fair, multiple zeros and some non-zeros is not max fair.
...
def _parse_as_date(column_name: Any) -> dt.date | None:
    ...
    if isinstance(column_name, (dt.date, dt.datetime, pd.Timestamp)): # ★ dt.date を先頭に
    ...
    if isinstance(column_name, str):
        ...
        try:
            ...
            try:
                ...
                if excel_serial > 0 and excel_serial < 200000: # Excelシリアルの妥当な範囲を広めに設定
    ...
    if isinstance(column_name, (int, float)): # Excelシリアル値
        try:
            ...
            if column_name > 0 and column_name < 200000: # 200000は約2444年なので十分なはず
                 return (datetime(1899, 12, 30) + timedelta(days=int(column_name))).date()
