...
def make_summary(heat_df: pd.DataFrame, by_role: bool = False) -> pd.DataFrame:
    ...
    if by_role and isinstance(heat_df.columns, pd.MultiIndex):
        ...
        out = (
            ...
            .groupby(
                level=[heat_df.index.name, heat_df.columns.names[1]]
            )  # timeとroleでグループ化
            .apply(
                _stats, include_groups=False
            )  # include_groups=False for pandas >= 2.0
        )
    else:
        # index = time
...
def build_staff_stats(long_df: pd.DataFrame, out_dir: Path) -> Path:
    ...
    * summary   : 列方向 5 数要約
    ...
    if (
        ...
        or "code" not in long_df.columns
        or "ds" not in long_df.columns
    ):
        log.error(
    ...
    staff_df["night_ratio"] = (
        (staff_df["night_shifts"] / staff_df["total_shifts"].replace(0, pd.NA))
        .fillna(0)
        .round(3)
    )
    ...
    def _five(col: pd.Series) -> pd.Series:
        ...
        if not pd.api.types.is_numeric_dtype(col):
            log.warning(
                f"[build_staff_stats] _five: 数値型でない列 '{col.name}' の統計量は計算できません。"
            )
            return pd.Series(
                {
                    "max": pd.NA,
