    ...
    def _calculate_key_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if shortage_file.exists():
                ...
                # 総不足時間（30分単位 → 時間換算）
                metrics['total_shortage_hours'] = round(shortage_df.sum().sum() * SLOT_HOURS, 1)
                
                # 平均日次不足
                if len(shortage_df.columns) > 0:
                    metrics['avg_daily_shortage'] = round(shortage_df.sum(axis=0).mean() * SLOT_HOURS, 1)
                ...
                if not timeslot_totals.empty:
                    ...
                    metrics['peak_shortage_timeslot'] = {
                        'timeslot': peak_index,
                        'shortage_hours': round(timeslot_totals[peak_index] * SLOT_HOURS, 1)
                    }
