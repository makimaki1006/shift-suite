...
def cluster_staff(long_df: pd.DataFrame, out_dir: Path, k: int = 3):
    if "staff" not in long_df.columns:
        log.error("[cluster] long_dfに 'staff' 列が見つかりません。処理をスキップします。")
        empty_cluster_df = pd.DataFrame(columns=["n_codes", "cluster"])
        save_df_xlsx(empty_cluster_df, out_dir / "staff_cluster.xlsx", "cluster")
        log.info("cluster: 'staff'列がなかったため、空のstaff_cluster.xlsxを作成しました。")
        return empty_cluster_df
    if "code" not in long_df.columns:
        log.error("[cluster] long_dfに 'code' 列が見つかりません。処理をスキップします。")
        empty_cluster_df = pd.DataFrame(columns=["n_codes", "cluster"])
        save_df_xlsx(empty_cluster_df, out_dir / "staff_cluster.xlsx", "cluster")
        log.info("cluster: 'code'列がなかったため、空のstaff_cluster.xlsxを作成しました。")
        return empty_cluster_df
    ...
    if feat.empty:
        log.warning("[cluster] 特徴量データ(feat)が空です。クラスタリングは実行されません。")
        empty_cluster_df = pd.DataFrame(columns=["n_codes", "cluster"])
        save_df_xlsx(empty_cluster_df, out_dir / "staff_cluster.xlsx", "cluster")
        return empty_cluster_df
    ...
    if len(feat) == 1:
        log.warning("[cluster] 特徴量データの行数が1のため、クラスタリングは実行せず、cluster 0 を割り当てます。")
        feat["cluster"] = 0
    ...
    if actual_k <= 0 : # サンプル数が0またはkが0以下の場合
        log.warning(f"[cluster] 有効なクラスタ数 k ({actual_k}) が0以下です。クラスタリングは実行されません。")
        feat["cluster"] = pd.NA # または適切なデフォルト値
        save_df_xlsx(feat, out_dir / "staff_cluster.xlsx", "cluster")
        return feat

    km = KMeans(n_clusters=actual_k, random_state=0, n_init='auto').fit(X) # n_init='auto'で警告抑制
