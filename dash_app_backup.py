        ...
        if not long_df.empty and 'parsed_slots_count' in long_df.columns:
            ...
            if not leave_data.empty:
                ...
                if 'parsed_slots_count' in long_df.columns:
                    ...
                    if zero_count == 0:
                        content.append(html.P("⚠️ このデータには休暇記録（parsed_slots_count=0）が含まれていません。", style={'color': 'orange'}))
                        # holiday_type 列をチェック
                    ...
                    content.append(html.P("⚠️ parsed_slots_count 列が存在しません。", style={'color': 'orange'}))
                    ...
                    if leave_csv_file.exists():
                        ...
                        try:
                            ...
                            for alt_scenario_dir in all_scenario_dirs:
                                ...
                                if alt_leave_file.exists():
                                    ...
                                    if not alt_leave_df.empty:
                                        content.append(html.P(f"✅ {alt_scenario_dir.name}/leave_analysis.csv からデータを読み込みました。", style={'color': 'green'}))
                            ...
                            if not found_alternative:
                                content.append(html.P("❌ 全てのシナリオで leave_analysis.csv が見つかりませんでした。", style={'color': 'red'}))
...
def create_fatigue_tab() -> html.Div:
    ...
    *デフォルトでは、これらの要素は均等な重み（各1.0）で評価されます。*
    ...
    if not df_fatigue.empty:
        ...
        content.append(html.Div([
            html.H4("⚠️ 疲労分析データが見つかりません", style={'color': '#e74c3c', 'marginBottom': '15px'}),
            ...
            html.P("💡 ヒント: 疲労分析にはスタッフの勤務データ（name/staff列）が必要です。", 
                   style={'fontStyle': 'italic', 'color': '#666', 'backgroundColor': '#f8f9fa', 'padding': '10px', 'borderRadius': '5px'})
        ], style={'padding': '20px', 'backgroundColor': '#fff3cd', 'borderRadius': '8px', 'border': '1px solid #ffeaa7'}))
...
def create_mece_facts_analysis_tab() -> html.Div:
    ...
    return html.Div([
        html.H3("📋 MECE事実抽出 (軸1: 施設ルール)", style={'marginBottom': '20px'}),
        html.P([
        ...
        dcc.Loading(
            ...
            children=html.Div([
                ...
                dcc.Tabs(id='mece-result-tabs', children=[
                    dcc.Tab(label='📊 抽出サマリー', value='summary', children=[
                        html.Div(id='mece-summary-content', style={'padding': '20px'})
                    ]),
                    dcc.Tab(label='🎯 確信度別分類', value='confidence', children=[
                        html.Div(id='mece-confidence-content', style={'padding': '20px'})
                    ]),
                    dcc.Tab(label='🔍 MECE分解事実', value='mece_details', children=[
                        html.Div(id='mece-details-content', style={'padding': '20px'})
                    ]),
                    dcc.Tab(label='🤖 AI実行用制約', value='constraints', children=[
                        html.Div(id='mece-constraints-content', style={'padding': '20px'})
                    ]),
                    dcc.Tab(label='📈 メタデータ', value='metadata', children=[
                        html.Div(id='mece-metadata-content', style={'padding': '20px'})
                    ])
                ], value='summary'),
                ...
                ], style={'marginTop': '30px', 'padding': '20px', 'backgroundColor': '#f5f5f5', 'borderRadius': '8px'})
        ...
        dcc.Loading(
            ...
            children=html.Div([
                ...
                dcc.Tabs(id='staff-result-tabs', children=[
                    dcc.Tab(label='📊 抽出サマリー', value='summary', children=[
                        html.Div(id='staff-summary-content', style={'padding': '20px'})
                    ]),
                    dcc.Tab(label='🎯 確信度別分類', value='confidence', children=[
                        html.Div(id='staff-confidence-content', style={'padding': '20px'})
                    ]),
                    dcc.Tab(label='🔍 MECE分解事実', value='mece_details', children=[
                        html.Div(id='staff-details-content', style={'padding': '20px'})
                    ]),
                    dcc.Tab(label='🤖 AI実行用制約', value='constraints', children=[
                        html.Div(id='staff-constraints-content', style={'padding': '20px'})
                    ]),
                    dcc.Tab(label='📈 メタデータ', value='metadata', children=[
                        html.Div(id='staff-metadata-content', style={'padding': '20px'})
                    ])
                ], value='summary'),
        ...
        dcc.Loading(
            ...
            children=html.Div([
                ...
                dcc.Tabs(id='time-calendar-result-tabs', children=[
                    dcc.Tab(label='📊 抽出サマリー', value='summary', children=[
                        html.Div(id='time-calendar-summary-content', style={'padding': '20px'})
                    ]),
                    dcc.Tab(label='🎯 確信度別分類', value='confidence', children=[
                        html.Div(id='time-calendar-confidence-content', style={'padding': '20px'})
                    ]),
                    dcc.Tab(label='🔍 MECE分解事実', value='mece_details', children=[
                        html.Div(id='time-calendar-details-content', style={'padding': '20px'})
                    ]),
                    dcc.Tab(label='🤖 AI実行用制約', value='constraints', children=[
                        html.Div(id='time-calendar-constraints-content', style={'padding': '20px'})
                    ]),
                    dcc.Tab(label='📈 メタデータ', value='metadata', children=[
                        html.Div(id='time-calendar-metadata-content', style={'padding': '20px'})
                    ])
                ], value='summary'),
        ...
        dcc.Loading(
            ...
            children=html.Div([
                ...
                dcc.Tabs(id='integration-result-tabs', children=[
                    dcc.Tab(label='📊 統合サマリー', value='summary', children=[
                        html.Div(id='integration-summary-content', style={'padding': '20px'})
                    ]),
                    dcc.Tab(label='🎯 制約階層', value='hierarchy', children=[
                        html.Div(id='integration-hierarchy-content', style={'padding': '20px'})
                    ]),
                    dcc.Tab(label='🔍 関係性分析', value='relationships', children=[
                        html.Div(id='integration-relationships-content', style={'padding': '20px'})
                    ]),
                    dcc.Tab(label='⚠️ 競合解決', value='conflicts', children=[
                        html.Div(id='integration-conflicts-content', style={'padding': '20px'})
                    ]),
                    dcc.Tab(label='🤖 統合制約データ', value='integrated_constraints', children=[
                        html.Div(id='integration-constraints-content', style={'padding': '20px'})
                    ]),
                    dcc.Tab(label='📈 メタデータ', value='metadata', children=[
                        html.Div(id='integration-metadata-content', style={'padding': '20px'})
                    ])
                ], value='summary'),
        ...
        dcc.Loading(
            ...
            children=html.Div([
                ...
                dcc.Tabs(id='three-axis-result-tabs', children=[
                    dcc.Tab(label='📊 統合サマリー', value='summary', children=[
                        html.Div(id='three-axis-summary-content', style={'padding': '20px'})
                    ]),
                    dcc.Tab(label='🎯 制約階層', value='hierarchy', children=[
                        html.Div(id='three-axis-hierarchy-content', style={'padding': '20px'})
                    ]),
                    dcc.Tab(label='🔍 3軸関係性分析', value='relationships', children=[
                        html.Div(id='three-axis-relationships-content', style={'padding': '20px'})
                    ]),
                    dcc.Tab(label='⚠️ 競合解決', value='conflicts', children=[
                        html.Div(id='three-axis-conflicts-content', style={'padding': '20px'})
                    ]),
                    dcc.Tab(label='🤖 統合制約データ', value='integrated_constraints', children=[
                        html.Div(id='three-axis-constraints-content', style={'padding': '20px'})
                    ]),
                    dcc.Tab(label='📈 メタデータ', value='metadata', children=[
                        html.Div(id='three-axis-metadata-content', style={'padding': '20px'})
                    ])
                ], value='summary'),
...
def create_main_ui_tabs():
    ...
    tabs = dcc.Tabs(id='main-tabs', value='overview', children=[
        ...
        dcc.Tab(label='📋 MECE事実抽出', value='mece_facts'),
        dcc.Tab(label='👤 職員ルール抽出', value='staff_rules'),
        dcc.Tab(label='🕐 時間カレンダールール', value='time_calendar_rules'),
        dcc.Tab(label='🔗 軸1+軸2統合', value='integrated_constraints'),
        dcc.Tab(label='🏗️ 3軸統合制約', value='three_axis_integration'),
    ...
    main_layout = html.Div([
        ...
        html.Div(style={'marginTop': '20px'}, children=[
            ...
            html.Div(id='mece-facts-tab-container',
                    style={'display': 'none'},
                    children=[
                        dcc.Loading(
                            id="loading-mece-facts",
                            type="circle",
                            children=html.Div(id='mece-facts-content')
                        )
                    ]),
            html.Div(id='staff-rules-tab-container',
                    style={'display': 'none'},
                    children=[
                        dcc.Loading(
                            id="loading-staff-rules",
                            type="circle",
                            children=html.Div(id='staff-rules-content')
                        )
                    ]),
            html.Div(id='time-calendar-rules-tab-container',
                    style={'display': 'none'},
                    children=[
                        dcc.Loading(
                            id="loading-time-calendar-rules",
                            type="circle",
                            children=html.Div(id='time-calendar-rules-content')
                        )
                    ]),
            html.Div(id='integrated-constraints-tab-container',
                    style={'display': 'none'},
                    children=[
                        dcc.Loading(
                            id="loading-integrated-constraints",
                            type="circle",
                            children=html.Div(id='integrated-constraints-content')
                        )
                    ]),
            html.Div(id='three-axis-integration-tab-container',
                    style={'display': 'none'},
                    children=[
                        dcc.Loading(
                            id="loading-three-axis-integration",
                            type="circle",
                            children=html.Div(id='three-axis-integration-content')
                        )
                    ]),
...
@app.callback(
    [Output('overview-tab-container', 'style'),
     ...
     Output('mece-facts-tab-container', 'style'),
     Output('staff-rules-tab-container', 'style'),
     Output('time-calendar-rules-tab-container', 'style'),
     Output('integrated-constraints-tab-container', 'style'),
     Output('three-axis-integration-tab-container', 'style'),
...
def update_tab_visibility(active_tab, selected_scenario, data_status):
    ...
    all_tabs = [
        ...
        'gap', 'individual_analysis', 'team_analysis', 'blueprint_analysis', 'mece_facts', 'staff_rules', 'time_calendar_rules', 'integrated_constraints', 'three_axis_integration', 'logic_analysis', 'ai_analysis'
    ]
...
@app.callback(
    Output('mece-facts-content', 'children'),
    Input('mece-facts-tab-container', 'style'),
    State('scenario-dropdown', 'value'),
    State('data-loaded', 'data'),
)
@safe_callback
def initialize_mece_facts_content(style, selected_scenario, data_status):
    """MECE事実抽出タブの内容を初期化"""
    if not selected_scenario or not data_status or style.get('display') == 'none':
        raise PreventUpdate
    try:
        return create_mece_facts_analysis_tab()
    except Exception as e:
        log.error(f"MECE事実抽出タブの初期化エラー: {str(e)}")
        return html.Div(f"エラーが発生しました: {str(e)}", style={'color': 'red'})
...
def update_blueprint_analysis_content(n_clicks, analysis_type):
    ...
    try:
        ...
        log.error(f"ブループリント分析エラー: {str(e)}")
        empty_fig = go.Figure()
        empty_fig.update_layout(title=f"エラーが発生しました: {str(e)}")
        error_message = html.Div(f"エラー: {str(e)}", style={'color': 'red'})
        return {}, empty_fig, [], [], [], error_message, error_message
...
def execute_mece_extraction(n_clicks, selected_scenario):
    ...
    try:
        # データ読み込み
        results = read_scenario_results(selected_scenario)
        long_df = results.get('original_data')
        
        if long_df is None or long_df.empty:
            error_msg = html.Div("分析対象データが見つかりません", style={'color': 'red'})
        ...
        # ステータス更新
        status_msg = html.Div([
            html.Span("✅ 抽出完了", style={'color': 'green', 'fontWeight': 'bold'}),
            html.Span(f" ({len(long_df)}レコード処理)", style={'color': '#666', 'marginLeft': '10px'})
        ])
        ...
        confidence_content = html.Div([
            ...
            ], style={'padding': '15px', 'margin': '10px 0', 'backgroundColor': '#e8f5e8', 'borderRadius': '8px'}),
            ...
            ], style={'padding': '15px', 'margin': '10px 0', 'backgroundColor': '#fff3e0', 'borderRadius': '8px'}),
            ...
            ], style={'padding': '15px', 'margin': '10px 0', 'backgroundColor': '#ffebee', 'borderRadius': '8px'})
        ])
        ...
        details_content = html.Div([
            ...
            html.Div([
                html.Div([
                    ...
                    html.P(f"...他{max(0, len(facts) - 5)}件", style={'color': '#666', 'fontStyle': 'italic'}) if len(facts) > 5 else html.Div()
                ], style={'margin': '15px 0', 'padding': '15px', 'border': '1px solid #ddd', 'borderRadius': '8px'})
...
# 軸2(職員ルール)抽出関連のコールバック
@app.callback(
    [Output('staff-results-store', 'data'),
     Output('staff-extraction-status', 'children'),
     Output('staff-summary-cards', 'children'),
     Output('staff-summary-content', 'children'),
     Output('staff-confidence-content', 'children'),
     Output('staff-details-content', 'children'),
     Output('staff-constraints-content', 'children'),
     Output('staff-metadata-content', 'children')],
    Input('execute-staff-extraction-button', 'n_clicks'),
    State('scenario-dropdown', 'value'),
    prevent_initial_call=True
)
@safe_callback
def execute_staff_extraction(n_clicks, selected_scenario):
    """軸2(職員ルール)抽出を実行"""
    if not n_clicks or not selected_scenario:
        raise PreventUpdate
    ...
    try:
        ...
        # ステータス更新
        status_msg = html.Div([
            html.Span("✅ 職員ルール抽出完了", style={'color': 'green', 'fontWeight': 'bold'}),
            html.Span(f" ({len(long_df)}レコード処理)", style={'color': '#666', 'marginLeft': '10px'})
        ])
        
        # サマリーカード作成
        human_readable = staff_results.get('human_readable', {})
        summary = human_readable.get('抽出事実サマリー', {})
        extraction_metadata = staff_results.get('extraction_metadata', {})
        data_quality = extraction_metadata.get('data_quality', {})
        
        summary_cards = html.Div([
            html.Div([
                html.H4(f"{summary.get('総事実数', 0)}", style={'margin': '0', 'color': '#388e3c'}),
                html.P("職員ルール総数", style={'margin': '5px 0 0 0', 'fontSize': '0.9rem'})
            ], style={'textAlign': 'center', 'padding': '20px', 'backgroundColor': '#e8f5e8', 'borderRadius': '8px'}),
            
            html.Div([
                html.H4(f"{summary.get('分析対象スタッフ数', 0)}", style={'margin': '0', 'color': '#1976d2'}),
                html.P("分析対象スタッフ", style={'margin': '5px 0 0 0', 'fontSize': '0.9rem'})
            ], style={'textAlign': 'center', 'padding': '20px', 'backgroundColor': '#e3f2fd', 'borderRadius': '8px'}),
            
            html.Div([
                html.H4(f"{len([k for k in summary.keys() if k not in ['総事実数', '分析対象スタッフ数']])}", style={'margin': '0', 'color': '#f57c00'}),
                html.P("分析カテゴリー", style={'margin': '5px 0 0 0', 'fontSize': '0.9rem'})
            ], style={'textAlign': 'center', 'padding': '20px', 'backgroundColor': '#fff3e0', 'borderRadius': '8px'}),
            
            html.Div([
                html.H4(f"{data_quality.get('analysis_coverage_ratio', 0):.1%}", style={'margin': '0', 'color': '#7b1fa2'}),
                html.P("カバレッジ率", style={'margin': '5px 0 0 0', 'fontSize': '0.9rem'})
            ], style={'textAlign': 'center', 'padding': '20px', 'backgroundColor': '#f3e5f5', 'borderRadius': '8px'})
        ], style={'display': 'grid', 'gridTemplateColumns': 'repeat(4, 1fr)', 'gap': '15px'})
        
        # 抽出サマリー内容
        summary_content = html.Div([
            html.H5("📊 軸2カテゴリー別抽出結果"),
            html.Div([
                html.Div([
                    html.Strong(f"{category}: "),
                ...
                for category, count in summary.items() if category not in ['総事実数', '分析対象スタッフ数']
            ])
        ])
        
        # その他のコンテンツは簡略化
        confidence_content = html.Div("確信度別分類データを表示します")
        details_content = html.Div("MECE分解事実詳細を表示します")
        constraints_content = html.Div("AI実行用制約データを表示します")
        metadata_content = html.Div("メタデータを表示します")
        
        return staff_results, status_msg, summary_cards, summary_content, confidence_content, details_content, constraints_content, metadata_content
        
    except Exception as e:
        log.error(f"職員ルール抽出エラー: {str(e)}")
        error_msg = html.Div(f"エラーが発生しました: {str(e)}", style={'color': 'red'})
        return {}, error_msg, html.Div(), html.Div(), html.Div(), html.Div(), html.Div(), html.Div()


# 軸1+軸2統合制約関連のコールバック
@app.callback(
    [Output('integration-results-store', 'data'),
     Output('integration-status', 'children'),
     Output('integration-summary-cards', 'children'),
     Output('integration-summary-content', 'children'),
     Output('integration-hierarchy-content', 'children'),
     Output('integration-relationships-content', 'children'),
     Output('integration-conflicts-content', 'children'),
     Output('integration-constraints-content', 'children'),
     Output('integration-metadata-content', 'children')],
    Input('execute-integration-button', 'n_clicks'),
    State('scenario-dropdown', 'value'),
    prevent_initial_call=True
)
@safe_callback
def execute_integration_analysis(n_clicks, selected_scenario):
    """軸1+軸2統合制約分析を実行"""
    if not n_clicks or not selected_scenario:
        raise PreventUpdate
    ...
    try:
        ...
        if long_df is None or long_df.empty:
            error_msg = html.Div("分析対象データが見つかりません", style={'color': 'red'})
            return {}, error_msg, html.Div(), html.Div(), html.Div(), html.Div(), html.Div(), html.Div(), html.Div()
        ...
        full_results = engine.run_full_blueprint_analysis(long_df, wt_df)
        
        # 統合結果取得
        integration_results = full_results.get('integrated_constraints', {})
        
        # ステータス更新
        status_msg = html.Div([
            html.Span("✅ 軸1+軸2統合分析完了", style={'color': 'green', 'fontWeight': 'bold'}),
            html.Span(f" ({len(long_df)}レコード処理)", style={'color': '#666', 'marginLeft': '10px'})
        ])
        
        # サマリーカード作成
        human_readable = integration_results.get('human_readable', {})
        machine_readable = integration_results.get('machine_readable', {})
        integration_metadata = integration_results.get('integration_metadata', {})
        
        summary_cards = html.Div([
            html.Div([
                html.H4(f"{len(machine_readable.get('hard_constraints', []))}", style={'margin': '0', 'color': '#d32f2f'}),
                html.P("統合ハード制約", style={'margin': '5px 0 0 0', 'fontSize': '0.9rem'})
            ], style={'textAlign': 'center', 'padding': '20px', 'backgroundColor': '#ffebee', 'borderRadius': '8px'}),
            
            html.Div([
                html.H4(f"{len(machine_readable.get('soft_constraints', []))}", style={'margin': '0', 'color': '#f57c00'}),
                html.P("統合ソフト制約", style={'margin': '5px 0 0 0', 'fontSize': '0.9rem'})
            ], style={'textAlign': 'center', 'padding': '20px', 'backgroundColor': '#fff3e0', 'borderRadius': '8px'}),
            
            html.Div([
                html.H4(f"{len(machine_readable.get('preferences', []))}", style={'margin': '0', 'color': '#388e3c'}),
                html.P("統合推奨設定", style={'margin': '5px 0 0 0', 'fontSize': '0.9rem'})
            ], style={'textAlign': 'center', 'padding': '20px', 'backgroundColor': '#e8f5e8', 'borderRadius': '8px'}),
            
            html.Div([
                html.H4(f"{integration_metadata.get('integration_quality', {}).get('completeness_score', 0):.1%}", style={'margin': '0', 'color': '#7b1fa2'}),
                html.P("統合完全性", style={'margin': '5px 0 0 0', 'fontSize': '0.9rem'})
            ], style={'textAlign': 'center', 'padding': '20px', 'backgroundColor': '#f3e5f5', 'borderRadius': '8px'})
        ], style={'display': 'grid', 'gridTemplateColumns': 'repeat(4, 1fr)', 'gap': '15px'})
        
        # その他のコンテンツは簡略化
        summary_content = html.Div("統合サマリーを表示します")
        hierarchy_content = html.Div("制約階層を表示します")
        relationships_content = html.Div("関係性分析を表示します")
        conflicts_content = html.Div("競合解決を表示します")
        constraints_content = html.Div("統合制約データを表示します")
        metadata_content = html.Div("統合メタデータを表示します")
        
        return integration_results, status_msg, summary_cards, summary_content, hierarchy_content, relationships_content, conflicts_content, constraints_content, metadata_content
        
    except Exception as e:
        log.error(f"統合制約分析エラー: {str(e)}")
        error_msg = html.Div(f"エラーが発生しました: {str(e)}", style={'color': 'red'})
        return {}, error_msg, html.Div(), html.Div(), html.Div(), html.Div(), html.Div(), html.Div(), html.Div()
...
# 軸3時間・カレンダールール抽出実行
@app.callback(
    [Output('time-calendar-results-store', 'data'),
     Output('time-calendar-extraction-status', 'children'),
     Output('time-calendar-summary-cards', 'children'),
     Output('time-calendar-summary-content', 'children'),
     Output('time-calendar-confidence-content', 'children'),
     Output('time-calendar-details-content', 'children'),
     Output('time-calendar-constraints-content', 'children'),
     Output('time-calendar-metadata-content', 'children')],
    Input('execute-time-calendar-extraction-button', 'n_clicks'),
    State('scenario-dropdown', 'value'),
    prevent_initial_call=True
)
@safe_callback
def execute_time_calendar_extraction(n_clicks, selected_scenario):
    """軸3(時間・カレンダールール)抽出を実行"""
    if not n_clicks or not selected_scenario:
        raise PreventUpdate
    ...
    try:
        ...
        time_calendar_extractor = TimeCalendarMECEFactExtractor()
        time_calendar_results = time_calendar_extractor.extract_axis3_time_calendar_rules(long_df, wt_df)
        
        # ステータス更新
        status_msg = html.Div([
            html.Span("✅ 時間・カレンダールール抽出完了", style={'color': 'green', 'fontWeight': 'bold'}),
            html.Span(f" ({len(long_df)}レコード処理)", style={'color': '#666', 'marginLeft': '10px'})
        ])
        
        # サマリーカード作成
        human_readable = time_calendar_results.get('human_readable', {})
        summary = human_readable.get('抽出事実サマリー', {})
        extraction_metadata = time_calendar_results.get('extraction_metadata', {})
        data_quality = extraction_metadata.get('data_quality', {})
        
        summary_cards = html.Div([
            html.Div([
                html.H4(f"{summary.get('総事実数', 0)}", style={'margin': '0', 'color': '#5d4037'}),
                html.P("時間制約総数", style={'margin': '5px 0 0 0', 'fontSize': '0.9rem'})
            ], style={'textAlign': 'center', 'padding': '20px', 'backgroundColor': '#efebe9', 'borderRadius': '8px'}),
            
            html.Div([
                html.H4(f"{summary.get('分析対象日数', 0)}", style={'margin': '0', 'color': '#1976d2'}),
                html.P("分析対象日数", style={'margin': '5px 0 0 0', 'fontSize': '0.9rem'})
            ], style={'textAlign': 'center', 'padding': '20px', 'backgroundColor': '#e3f2fd', 'borderRadius': '8px'}),
            
            html.Div([
                html.H4(f"{data_quality.get('temporal_coverage', 0):.1%}", style={'margin': '0', 'color': '#388e3c'}),
                html.P("時間カバレッジ", style={'margin': '5px 0 0 0', 'fontSize': '0.9rem'})
            ], style={'textAlign': 'center', 'padding': '20px', 'backgroundColor': '#e8f5e8', 'borderRadius': '8px'}),
            
            html.Div([
                html.H4(f"{data_quality.get('calendar_complexity', 0):.1f}", style={'margin': '0', 'color': '#f57c00'}),
                html.P("カレンダー複雑度", style={'margin': '5px 0 0 0', 'fontSize': '0.9rem'})
            ], style={'textAlign': 'center', 'padding': '20px', 'backgroundColor': '#fff3e0', 'borderRadius': '8px'})
        ], style={'display': 'grid', 'gridTemplateColumns': 'repeat(4, 1fr)', 'gap': '15px'})
        
        # その他のコンテンツは簡略化
        summary_content = html.Div("時間・カレンダールールサマリーを表示します")
        confidence_content = html.Div("確信度別分類を表示します")
        details_content = html.Div("MECE分解事実を表示します")
        constraints_content = html.Div("AI実行用制約を表示します")
        metadata_content = html.Div("メタデータを表示します")
        
        return time_calendar_results, status_msg, summary_cards, summary_content, confidence_content, details_content, constraints_content, metadata_content
        
    except Exception as e:
        log.error(f"時間・カレンダールール抽出エラー: {str(e)}")
        error_msg = html.Div(f"エラーが発生しました: {str(e)}", style={'color': 'red'})
        return {}, error_msg, html.Div(), html.Div(), html.Div(), html.Div(), html.Div(), html.Div()
...
# 3軸統合制約分析実行
@app.callback(
    [Output('three-axis-results-store', 'data'),
     Output('three-axis-integration-status', 'children'),
     Output('three-axis-summary-cards', 'children'),
     Output('three-axis-summary-content', 'children'),
     Output('three-axis-hierarchy-content', 'children'),
     Output('three-axis-relationships-content', 'children'),
     Output('three-axis-conflicts-content', 'children'),
     Output('three-axis-constraints-content', 'children'),
     Output('three-axis-metadata-content', 'children')],
    Input('execute-three-axis-integration-button', 'n_clicks'),
    State('scenario-dropdown', 'value'),
    prevent_initial_call=True
)
@safe_callback
def execute_three_axis_integration(n_clicks, selected_scenario):
    """3軸統合制約分析を実行"""
    if not n_clicks or not selected_scenario:
        raise PreventUpdate
    ...
    try:
        ...
        # 3軸統合結果取得
        three_axis_results = full_results.get('three_axis_integration', {})
        
        # ステータス更新
        status_msg = html.Div([
            html.Span("✅ 3軸統合分析完了", style={'color': 'green', 'fontWeight': 'bold'}),
            html.Span(f" ({len(long_df)}レコード処理)", style={'color': '#666', 'marginLeft': '10px'})
        ])
        
        # サマリーカード作成
        human_readable = three_axis_results.get('human_readable', {})
        machine_readable = three_axis_results.get('machine_readable', {})
        integration_metadata = three_axis_results.get('integration_metadata', {})
        
        summary_cards = html.Div([
            html.Div([
                html.H4(f"{len(machine_readable.get('hard_constraints', []))}", style={'margin': '0', 'color': '#d32f2f'}),
                html.P("3軸ハード制約", style={'margin': '5px 0 0 0', 'fontSize': '0.9rem'})
            ], style={'textAlign': 'center', 'padding': '20px', 'backgroundColor': '#ffebee', 'borderRadius': '8px'}),
            
            html.Div([
                html.H4(f"{len(machine_readable.get('soft_constraints', []))}", style={'margin': '0', 'color': '#f57c00'}),
                html.P("3軸ソフト制約", style={'margin': '5px 0 0 0', 'fontSize': '0.9rem'})
            ], style={'textAlign': 'center', 'padding': '20px', 'backgroundColor': '#fff3e0', 'borderRadius': '8px'}),
            
            html.Div([
                html.H4(f"{len(machine_readable.get('preferences', []))}", style={'margin': '0', 'color': '#388e3c'}),
                html.P("3軸推奨設定", style={'margin': '5px 0 0 0', 'fontSize': '0.9rem'})
            ], style={'textAlign': 'center', 'padding': '20px', 'backgroundColor': '#e8f5e8', 'borderRadius': '8px'}),
            
            html.Div([
                html.H4(f"{integration_metadata.get('integration_quality', {}).get('completeness_score', 0):.1%}", style={'margin': '0', 'color': '#e65100'}),
                html.P("3軸統合完全性", style={'margin': '5px 0 0 0', 'fontSize': '0.9rem'})
            ], style={'textAlign': 'center', 'padding': '20px', 'backgroundColor': '#fbe9e7', 'borderRadius': '8px'})
        ], style={'display': 'grid', 'gridTemplateColumns': 'repeat(4, 1fr)', 'gap': '15px'})
        
        # その他のコンテンツは簡略化
        summary_content = html.Div("3軸統合サマリーを表示します")
        hierarchy_content = html.Div("3軸制約階層を表示します")
        relationships_content = html.Div("3軸関係性分析を表示します")
        conflicts_content = html.Div("3軸競合解決を表示します")
        constraints_content = html.Div("3軸統合制約データを表示します")
        metadata_content = html.Div("3軸統合メタデータを表示します")
        
        return three_axis_results, status_msg, summary_cards, summary_content, hierarchy_content, relationships_content, conflicts_content, constraints_content, metadata_content
        
    except Exception as e:
        log.error(f"3軸統合制約分析エラー: {str(e)}")
        error_msg = html.Div(f"エラーが発生しました: {str(e)}", style={'color': 'red'})
        return {}, error_msg, html.Div(), html.Div(), html.Div(), html.Div(), html.Div(), html.Div(), html.Div()
...
# 軸2職員ルール制約エクスポート機能
@app.callback(
    Output('download-staff-constraints', 'data'),
    Input('export-staff-constraints-button', 'n_clicks'),
    State('staff-results-store', 'data'),
    prevent_initial_call=True
)
@safe_callback
def export_staff_constraints_data(n_clicks, staff_results):
    """軸2職員ルール制約データをJSONでエクスポート"""
    if not n_clicks or not staff_results:
        raise PreventUpdate
    
    machine_readable = staff_results.get('machine_readable', {})
    
    import datetime as dt
    timestamp = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"staff_constraints_{timestamp}.json"
...
@app.callback(
    Output('download-staff-report', 'data'),
    Input('export-staff-report-button', 'n_clicks'),
    State('staff-results-store', 'data'),
    prevent_initial_call=True
)
@safe_callback
def export_staff_report_data(n_clicks, staff_results):
    """軸2職員ルールレポートをJSONでエクスポート"""
    if not n_clicks or not staff_results:
        raise PreventUpdate
    ...
    report_data = {
        ...
        "constraints_preview": {
            "staff_hard_constraints_count": len(machine_readable.get("staff_hard_constraints", [])),
            "staff_soft_constraints_count": len(machine_readable.get("staff_soft_constraints", [])),
            "staff_preferences_count": len(machine_readable.get("staff_preferences", []))
        }
    }
    
    timestamp = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"staff_rules_report_{timestamp}.json"
...
# 軸1+軸2統合制約エクスポート機能
@app.callback(
    Output('download-integration-constraints', 'data'),
    Input('export-integration-constraints-button', 'n_clicks'),
    State('integration-results-store', 'data'),
    prevent_initial_call=True
)
@safe_callback
def export_integration_constraints_data(n_clicks, integration_results):
    """統合制約データをJSONでエクスポート"""
    if not n_clicks or not integration_results:
        raise PreventUpdate
    ...
    import datetime as dt
    timestamp = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"integrated_constraints_{timestamp}.json"
...
@app.callback(
    Output('download-integration-report', 'data'),
    Input('export-integration-report-button', 'n_clicks'),
    State('integration-results-store', 'data'),
    prevent_initial_call=True
)
@safe_callback
def export_integration_report_data(n_clicks, integration_results):
    """統合分析レポートをJSONでエクスポート"""
    if not n_clicks or not integration_results:
        raise PreventUpdate
    ...
    report_data = {
        "report_metadata": {
            "generated_at": dt.datetime.now().isoformat(),
            "report_type": "軸1+軸2統合制約分析レポート",
            "axes": [1, 2],
            "application": "Shift-Suite Dashboard",
            "integration_timestamp": integration_metadata.get("integration_timestamp", ""),
            "integration_quality": integration_metadata.get("integration_quality", {})
        },
        "integration_summary": human_readable.get('統合サマリー', {}),
        "constraint_analysis": human_readable.get('制約統合分析', {}),
        "execution_priority": human_readable.get('実行優先度', []),
        "review_items": human_readable.get('要確認事項', []),
        "constraints_overview": {
            ...
            "relationships_count": len(machine_readable.get("constraint_relationships", [])),
            "conflicts_detected": len(machine_readable.get("conflict_resolution", {}).get("detected_conflicts", []))
        },
        "axis_coverage": integration_metadata.get("axis_coverage", {}),
        "constraint_distribution": integration_metadata.get("constraint_distribution", {})
    }
    
    timestamp = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"integration_analysis_report_{timestamp}.json"
...
# 軸3時間・カレンダールール制約エクスポート機能
@app.callback(
    Output('download-time-calendar-constraints', 'data'),
    Input('export-time-calendar-constraints-button', 'n_clicks'),
    State('time-calendar-results-store', 'data'),
    prevent_initial_call=True
)
@safe_callback
def export_time_calendar_constraints_data(n_clicks, time_calendar_results):
    """軸3時間・カレンダールール制約データをJSONでエクスポート"""
    if not n_clicks or not time_calendar_results:
        raise PreventUpdate
    
    machine_readable = time_calendar_results.get('machine_readable', {})
    
    import datetime as dt
    timestamp = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"time_calendar_constraints_{timestamp}.json"
...
@app.callback(
    Output('download-time-calendar-report', 'data'),
    Input('export-time-calendar-report-button', 'n_clicks'),
    State('time-calendar-results-store', 'data'),
    prevent_initial_call=True
)
@safe_callback
def export_time_calendar_report_data(n_clicks, time_calendar_results):
    """軸3時間・カレンダールール人間確認用レポートをJSONでエクスポート"""
    if not n_clicks or not time_calendar_results:
        raise PreventUpdate
    ...
    report_data = {
        "report_metadata": {
            "generated_at": dt.datetime.now().isoformat(),
            "report_type": "MECE時間・カレンダールール事実抽出レポート",
            "axis": 3,
            "application": "Shift-Suite Dashboard",
            "data_period": extraction_metadata.get("data_period", {}),
            "data_quality": extraction_metadata.get("data_quality", {})
        ...
        "detailed_facts": human_readable.get('MECE分解事実', {}),
        "constraints_preview": {
            "time_hard_constraints_count": len(machine_readable.get("time_hard_constraints", [])),
            "time_soft_constraints_count": len(machine_readable.get("time_soft_constraints", [])),
            "time_preferences_count": len(machine_readable.get("time_preferences", []))
        }
    }
    
    timestamp = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"time_calendar_rules_report_{timestamp}.json"
...
# 3軸統合制約エクスポート機能
@app.callback(
    Output('download-three-axis-constraints', 'data'),
    Input('export-three-axis-constraints-button', 'n_clicks'),
    State('three-axis-results-store', 'data'),
    prevent_initial_call=True
)
@safe_callback
def export_three_axis_constraints_data(n_clicks, three_axis_results):
    """3軸統合制約データをJSONでエクスポート"""
    if not n_clicks or not three_axis_results:
        raise PreventUpdate
    ...
    import datetime as dt
    timestamp = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"three_axis_constraints_{timestamp}.json"
...
@app.callback(
    Output('download-three-axis-report', 'data'),
    Input('export-three-axis-report-button', 'n_clicks'),
    State('three-axis-results-store', 'data'),
    prevent_initial_call=True
)
@safe_callback
def export_three_axis_report_data(n_clicks, three_axis_results):
    """3軸統合人間確認用レポートをJSONでエクスポート"""
    if not n_clicks or not three_axis_results:
        raise PreventUpdate
    ...
    report_data = {
        "report_metadata": {
            "generated_at": dt.datetime.now().isoformat(),
            "report_type": "3軸統合制約分析レポート",
            "axes": [1, 2, 3],
            "application": "Shift-Suite Dashboard",
            "data_period": integration_metadata.get("data_period", {}),
            "integration_quality": integration_metadata.get("integration_quality", {})
        },
        "summary": human_readable.get('統合サマリー', {}),
        "constraint_analysis": human_readable.get('制約統合分析', {}),
        "execution_priority": human_readable.get('実行優先度', []),
        "review_items": human_readable.get('要確認事項', []),
        "constraints_overview": {
        ...
        "axis_coverage": integration_metadata.get("axis_coverage", {}),
        "constraint_distribution": integration_metadata.get("constraint_distribution", {}),
        "three_axis_relationships": integration_metadata.get("three_axis_relationships", {})
    }
    
    timestamp = dt.datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"three_axis_integration_report_{timestamp}.json"
...
@safe_callback
def export_report_data(n_clicks, mece_results):
    """人間確認用レポートをJSONでエクスポート"""
    if not n_clicks or not mece_results:
        raise PreventUpdate
...
def find_related_staff(rule_text, staff_names):
...
@safe_callback
def display_staff_radar_and_rules(selected_staff, selected_rows, stored_blueprint_data, table_data):
    """スタッフレーダーチャートと関連ルールを表示"""
    try:
        empty_fig = go.Figure()
        empty_rules = html.Div("スタッフを選択してください")
        
        if not stored_blueprint_data:
            return empty_fig, empty_rules
            
        # 簡単な実装を返す
        return empty_fig, empty_rules
        
    except Exception as e:
        log.error(f"スタッフレーダー表示エラー: {str(e)}")
        return go.Figure(), html.Div(f"エラー: {str(e)}", style={'color': 'red'})
...
@app.callback(
    ...
    [Input('individual-selector', 'value'),
     Input('individual-metrics-checklist', 'value')],
    State('data-loaded', 'data'),
    prevent_initial_call=True
)
@safe_callback
def update_individual_analysis(selected_employee, selected_metrics, data_status):
    """個人分析を更新"""
    if not selected_employee or not data_status:
        return html.Div("職員を選択してください")
    
    try:
        # 個人分析の簡単な実装
        return html.Div([
            html.H4(f"{selected_employee}の分析"),
            html.P("個人分析データを表示します")
        ])
...
@app.callback(
    Output('log-viewer', 'value'),
    Input('log-interval', 'n_intervals')
)
def update_log_viewer(n_intervals):
    """ログビューアーを更新"""
    try:
        return "ログ情報を表示します"
    except:
...
@app.callback(
    [Output('individual-selector', 'options'),
     Output('individual-selector', 'value')],
    Input('scenario-dropdown', 'value'),
    State('data-loaded', 'data')
)
@safe_callback
def update_individual_selector(selected_scenario, data_status):
    """個人選択肢を更新"""
    if not selected_scenario or not data_status:
        return [], None
    ...
    try:
        # 簡単な実装
        options = [{'label': f'職員{i}', 'value': f'staff{i}'} for i in range(1, 6)]
