...
def create_shortage_tab(selected_scenario: str = None) -> html.Div:
    ...
    try:
        ...
        if "df_shortage_role_filtered" in str(e):
            shortage_dash_log.error("[WARNING] df_shortage_role_filteredエラーを検出！")
...
def create_proportional_abolition_tab(selected_scenario: str = None) -> html.Div:
    ...
    try:
        ...
        # [TARGET] 分析タイトルとコンセプト説明
        content.append(html.Div([
            html.H2("[TARGET] 按分廃止・職種別分析", style=UNIFIED_STYLES['header']),
            html.P([
        ...
        if df_proportional_role.empty:
            ...
            content.append(html.Div([
                html.H4("[WARNING] データ不足", style={'color': '#f59e0b', 'fontWeight': '600'}),
                html.P([
                    "按分廃止分析結果が見つかりません。",
                    html.Br(),
        ...
        if not df_proportional_org.empty:
            ...
            content.append(html.H3("[CHART] 組織全体の真の過不足状況"))
        ...
        # [TARGET] 職種別真の過不足分析
        content.append(html.H3("[TARGET] 職種別真の過不足分析 (按分廃止)", 
        ...
        if not df_proportional_role.empty:
            ...
            if len(shortage_roles) > 0:
                ...
                if len(severe_shortage) > 0 or len(zero_allocation) > 0:
                    content.append(html.Div([
                        html.H4("[WARNING] 按分廃止により発見された重要な問題", style={'color': '#f44336'}),
                        html.Ul([
            ...
            # 改善アクションプラン
            if len(shortage_roles) > 0:
                content.append(html.H4("[TARGET] 改善アクションプラン"))
...
def create_main_ui_tabs():
    ...
    tabs = dcc.Tabs(
        ...
        dcc.Tab(label='[GRAPH] 需要予測', value='forecast'),
        dcc.Tab(label='👷 採用計画', value='hire_plan'),
        dcc.Tab(label='💰 コスト分析', value='cost'),
        
        # 高度分析グループ
        dcc.Tab(label='[BOARD] 基準乖離分析', value='gap'),
