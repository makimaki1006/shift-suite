...
try:
    ...
    log.info("📊 Phase 3 統合ファクトブック分析が利用可能です")
except ImportError as e:
...
def create_upload_area() -> html.Div:
    ...
    return html.Div([
        ...
        html.Div([
            ...
            # 使用方法の説明
            html.Div([
                html.H4("📚 使用方法", style={'color': '#2c3e50', 'marginBottom': '10px'}),
                html.Ol([
    ...
    try:
        ...
        if 'total_shortage_hours' not in kpis or kpis['total_shortage_hours'] == 0:
            ...
            if shortage_role_file.exists():
                ...
                # 🔧 修正: emp_で始まる職種（雇用形態の誤混入）を除外
                if 'role' in df.columns:
                    # emp_で始まる行を除外してログ出力
                    emp_roles = df[df['role'].str.startswith('emp_', na=False)]
                    if not emp_roles.empty:
                        log.warning(f"⚠️ 雇用形態が職種として混入: {emp_roles['role'].tolist()}")
                ...
                if total_shortage > 10000:  # 大きな値の場合は参考情報を表示
                    ...
                    log.info(f"📊 大規模データセット検出: {total_shortage:.0f}時間")
...
if UNIFIED_ANALYSIS_AVAILABLE:
    ...
    log.info("🔧 グローバル統一分析マネージャーを初期化")
else:
    global_unified_manager = None

# 🔧 新機能: 動的スロット時間計算（SLOT_HOURS固定値の代替）
def get_dynamic_slot_hours(slot_minutes: int = None) -> float:
    ...
    log.debug(f"🔧 動的スロット時間計算: {slot_minutes}分 = {slot_hours:.2f}時間")
    return slot_hours
...
def get_unified_analysis_data(file_pattern: str, scenario: str = "median_based") -> Dict[str, Any]:
    ...
    if not UNIFIED_ANALYSIS_AVAILABLE or global_unified_manager is None:
        log.debug("🔧 統一システム利用不可 - parquetフォールバックのみ")
        return {}
    ...
    try:
        ...
        if unified_results:
            log.info(f"🔧 統一システムから{len(unified_results)}種類の結果を取得: {file_pattern} ({scenario})")
            return unified_results
        else:
            log.warning(f"🔧 統一システムから結果取得失敗: {file_pattern} ({scenario})")
            return {}
            
    except Exception as e:
        log.error(f"🔧 統一システムエラー: {e}")
        return {}
        ...
        html.Details([
            ...
            html.Div([
                ...
                **統計的手法:**
                ...
                - スロット単位: {DEFAULT_SLOT_MINUTES}分 = {get_dynamic_slot_hours():.2f}時間
...
def create_mece_constraint_system_tab() -> html.Div:
    ...
    return html.Div([
        html.H3("📋 MECE制約抽出システム", style={'marginBottom': '20px'}),
        html.P([
        ...
        html.Div([
            ...
            dcc.RadioItems(
                ...
                options=[
                    ...
                    {'label': '📊 軸11: パフォーマンス・改善', 'value': 'axis11'},
    ...
    try:
        ...
        success_message = html.Div([
            html.Div([
                ...
                html.H3("✅ アップロード成功!", style={'color': '#27ae60', 'marginBottom': '10px'}),
        ...
        error_message = html.Div([
            html.Div([
                ...
                html.H3("⚠️ ファイルエラー", style={'color': '#e74c3c', 'marginBottom': '10px'}),
                html.P("破損したZIPファイルです", style={'marginBottom': '10px'}),
                html.P("ファイルが正しくダウンロードされているか確認してください。", style={'color': '#666'})
            ], style={'textAlign': 'center', 'padding': '30px'})
        ], style={
        ...
        error_message = html.Div([
            html.Div([
                ...
                html.H3("❌ 処理エラー", style={'color': '#e74c3c', 'marginBottom': '10px'}),
                html.P("ファイル処理中にエラーが発生しました", style={'marginBottom': '10px'}),
                html.Details([
                    html.Summary("エラー詳細を表示", style={'cursor': 'pointer', 'color': '#666'}),
                    html.P(str(e), style={'backgroundColor': '#f8f9fa', 'padding': '10px', 'borderRadius': '4px', 'fontFamily': 'monospace'})
                ]),
                html.P("ファイル形式や内容を確認してください。", style={'color': '#666', 'marginTop': '10px'})
            ], style={'textAlign': 'center', 'padding': '30px'})
        ], style={
            'backgroundColor': '#f8d7da',
            'border': '1px solid #f5c6cb',
...
def create_main_ui_tabs():
    ...
    tabs = dcc.Tabs(id='main-tabs', value='overview', children=[
        ...
        dcc.Tab(label='📋 MECE制約抽出システム', value='mece_constraint_system'),
        dcc.Tab(label='🔍 ロジック解明', value='logic_analysis'),
        
        # 真実発見グループ
        dcc.Tab(label='🎯 真実あぶり出し', value='truth_discovery') if DIRECT_TRUTH_AVAILABLE else None,
    ])
...
def initialize_fact_book_analysis_content(style, selected_scenario, data_status):
    ...
    try:
        if FACT_BOOK_INTEGRATION_AVAILABLE:
            ...
            return html.Div([
                html.H3("📊 統合ファクトブック分析", style={'color': '#2c3e50'}),
...
def create_facts_analysis_display(results: dict) -> html.Div:
    ...
    if not objective_facts:
        ...
        return html.Div([
            html.H4("📋 客観的事実分析結果"),
            html.Div([
                html.P(f"⚠️ {error_msg}", style={'color': 'orange', 'textAlign': 'center', 'padding': '20px'})
    ...
    return html.Div([
        ...
        # メタデータ表示
        html.Div([
            html.P(f"📊 総事実数: {analysis_metadata.get('total_facts', 0)}件 | " +
...
def create_integrated_analysis_display(results: dict) -> html.Div:
    ...
    # 統合概要タブ
    tabs.append(
        dcc.Tab(label='📊 統合概要', children=[
            html.Div([
                html.H4("🔄 統合分析結果（暗黙知 + 客観的事実）"),
                
                # 分析概要
                html.Div([
                    html.P(f"📊 暗黙知パターン: {analysis_metadata.get('total_patterns', 0)}件 | " +
                ...
                html.Div([
                    html.Div([
                        html.H6(f"🎯 {insight.get('type', 'N/A')}", style={'color': '#1f77b4'}),
                        html.P(insight.get('insight', 'N/A')),
                        html.P(f"📋 推奨アクション: {insight.get('recommendation', 'N/A')}", 
                              style={'fontStyle': 'italic', 'color': '#666'}),
                        html.P(f"⭐ 優先度: {insight.get('priority', 'medium')}", 
                              style={'fontSize': '12px', 'color': '#888'})
                    ], style={'backgroundColor': '#f8f9fa', 'padding': '15px', 'borderRadius': '8px', 'marginBottom': '10px'})
                    for insight in integrated_insights
                ]) if integrated_insights else html.P("統合インサイトは見つかりませんでした", style={'color': '#666', 'fontStyle': 'italic'}),
                ...
                html.Div([
                    html.Div([
                        ...
                        html.P(f"🔗 関連強度: {rel.get('relationship_strength', 0):.2f}", 
                              style={'fontSize': '12px', 'color': '#888'})
                    ], style={'backgroundColor': '#fef9e7', 'padding': '15px', 'borderRadius': '8px', 'marginBottom': '10px'})
                    for rel in relationships[:5]  # 上位5件のみ表示
                ]) if relationships else html.P("関連性は見つかりませんでした", style={'color': '#666', 'fontStyle': 'italic'})
            ])
        ])
    )
    ...
    if objective_facts:
        tabs.append(
            dcc.Tab(label=f'📋 客観的事実 ({len(objective_facts)}件)', children=[
...
def create_basic_blueprint_analysis(long_df: pd.DataFrame, analysis_type: str) -> html.Div:
    ...
    try:
        ...
        return html.Div([
            html.H4(f"📊 基本分析結果 ({analysis_type})"),
            html.Div([
                html.Div([
                    html.H5("📊 データ概要"),
                ...
                ], style={'backgroundColor': '#fff3cd', 'padding': '20px', 'borderRadius': '8px', 'marginTop': '20px'})
            ])
        ])
        
    except Exception as e:
        return html.Div(f"基本分析エラー: {str(e)}", style={'color': 'red'})
...
def execute_unified_mece_analysis(n_clicks, selected_axis, selected_scenario):
    ...
    try:
        ...
        # ステータス更新
        status_msg = html.Div([
            html.Span(f"✅ {axis_title} 分析完了", style={'color': 'green', 'fontWeight': 'bold'}),
            html.Span(f" ({len(long_df)}レコード処理)", style={'color': '#666', 'marginLeft': '10px'})
        ])
...
def create_unified_constraints_content(analysis_results):
    ...
    if hard_constraints:
        constraint_sections.append(
            html.Div([
                ...
                html.P("AIが絶対に守らなければならない制約", style={'fontSize': '0.9rem', 'color': '#666'}),
                html.Div([
                    html.Div([
    ...
    if soft_constraints:
        constraint_sections.append(
            html.Div([
                html.H6("🔧 SOFT制約（推奨）", style={'color': '#ff9800', 'marginBottom': '10px'}),
                html.P("AIが可能な限り守ろうとする制約", style={'fontSize': '0.9rem', 'color': '#666'}),
                html.Div([
                    html.Div([
                        html.Code(json.dumps(constraint, indent=2, ensure_ascii=False), 
                                style={'display': 'block', 'whiteSpace': 'pre-wrap'})
    ...
    constraint_sections.append(
        html.Div([
            ...
            html.P("AIシステムでの利用方法例", style={'fontSize': '0.9rem', 'color': '#666'}),
            html.Div([
...
response = ai_shift_system.create_shift(
    ...
    return html.Div([
        ...
        html.P([
            "⚠️ ", html.Strong("注意："), "これは人間向けの説明文ではなく、AIが実行するための構造化データです。"
        ], style={'color': '#d32f2f', 'marginBottom': '20px', 'fontWeight': 'bold'}),
        html.Div(constraint_sections, style={'padding': '20px'})
    ])
...
def create_unified_metadata_content(analysis_results):
    ...
    if extraction_info:
        metadata_sections.append(
            html.Div([
                html.H6("📊 抽出基本情報", style={'color': '#1976d2', 'marginBottom': '10px'}),
                html.Div([
    ...
    if data_quality:
        ...
        metadata_sections.append(
            html.Div([
                html.H6("📋 データ品質指標", style={'color': '#388e3c', 'marginBottom': '10px'}),
                html.Div([
                    html.Div([
                    ...
                    ], style={'textAlign': 'center', 'padding': '15px', 'backgroundColor': '#e3f2fd', 'borderRadius': '8px'}),
                    ...
                    html.Div([
                        html.H4(f"{staff_coverage.get('total_staff', 0)}", style={'margin': '0', 'color': '#ff9800'}),
                    ...
                    html.Div([
                        ...
                        html.P("データ完全性", style={'margin': '5px 0 0 0', 'fontSize': '0.9rem'})
                    ], style={'textAlign': 'center', 'padding': '15px', 'backgroundColor': '#f3e5f5', 'borderRadius': '8px'})
    ...
    if mece_analysis:
        metadata_sections.append(
            html.Div([
                html.H6("🔍 MECE分析結果", style={'color': '#ff9800', 'marginBottom': '10px'}),
                html.Div([
    ...
    if axis_relationships:
        metadata_sections.append(
            html.Div([
                ...
                ], style={'padding': '15px', 'backgroundColor': '#f3e5f5', 'borderRadius': '8px'})
            ], style={'marginBottom': '20px'})
        )
    ...
    if confidence_indicators:
        metadata_sections.append(
            html.Div([
                html.H6("📊 信頼度指標", style={'color': '#4caf50', 'marginBottom': '10px'}),
                html.Div([
                    html.Div([
    ...
    if limitations or recommendations:
        metadata_sections.append(
            html.Div([
                html.H6("⚠️ 制限事項と推奨事項", style={'color': '#d32f2f', 'marginBottom': '10px'}),
                html.Div([
                    html.Div([
                        html.Strong("制限事項:"),
                ...
                ], style={'padding': '15px', 'backgroundColor': '#ffebee', 'borderRadius': '8px'})
            ], style={'marginBottom': '20px'})
        )
    ...
    return html.Div([
        html.H5("📈 抽出メタデータ（extraction_metadata）"),
        html.P("分析品質・信頼度・制約数など、システム監視用のメタ情報です。", 
               style={'color': '#666', 'marginBottom': '20px'}),
        html.Div(metadata_sections, style={'padding': '20px'})
    ])
...
def update_axis_description(selected_axis):
    ...
    descriptions = {
        ...
        'axis4': html.Div([
            html.H5("📊 軸4: 需要・負荷管理", style={'color': '#1976d2'}),
        ...
        'axis11': html.Div([
            html.H5("📊 軸11: パフォーマンス・改善", style={'color': '#2196f3'}),
    ...
    def execute_truth_discovery_analysis(n_clicks, selected_scenario):
        ...
        try:
            ...
            results_div = html.Div([
                # サマリー
                html.Div([
                    html.H4("🎯 真実あぶり出し分析結果", style={'color': '#2c3e50', 'marginBottom': '15px'}),
                    html.Div([
                        html.Div([
                ...
                # 上位の意図
                html.Div([
                    html.H5("🔍 最も確信度の高いシフト作成者の意図（上位10件）", style={'color': '#2c3e50'}),
                    html.Div([
                        html.Div([
    ...
    try:
        if FACT_BOOK_INTEGRATION_AVAILABLE:
            ...
            result_layout = html.Div([
                html.H4("📊 統合ファクトブック分析結果", style={'color': '#2c3e50'}),
                html.P(f"分析対象シナリオ: {selected_scenario}", style={'marginBottom': '10px'}),
                html.P(f"異常検知感度: {sensitivity}", style={'marginBottom': '20px'}),
                
                # Phase統合状況の表示
                html.Div([
                    html.H5("🎯 Phase 3 機能統合完了状況", style={'color': '#27ae60'}),
                    html.Div([
                        html.Div([
                            html.H6("Phase 2: 基本事実抽出", style={'margin': '0', 'color': '#2c3e50'}),
                            html.P("✅ FactExtractorPrototype", style={'margin': '5px 0', 'color': '#27ae60'}),
                            html.P("✅ 基本勤務統計・パターン統計・組織統計", style={'margin': '5px 0', 'fontSize': '14px'})
                        ], style={'padding': '15px', 'border': '1px solid #2ecc71', 'borderRadius': '5px', 'margin': '10px', 'backgroundColor': '#f8fff8'}),
                        
                        html.Div([
                            html.H6("Phase 3.1: 異常検知", style={'margin': '0', 'color': '#2c3e50'}),
                            html.P("✅ LightweightAnomalyDetector", style={'margin': '5px 0', 'color': '#27ae60'}),
                            html.P("✅ 4つの異常検知アルゴリズム実装", style={'margin': '5px 0', 'fontSize': '14px'})
                        ], style={'padding': '15px', 'border': '1px solid #3498db', 'borderRadius': '5px', 'margin': '10px', 'backgroundColor': '#f8fbff'}),
                        
                        html.Div([
                            html.H6("Phase 3.2: 統合可視化", style={'margin': '0', 'color': '#2c3e50'}),
                            html.P("✅ FactBookVisualizer", style={'margin': '5px 0', 'color': '#27ae60'}),
                            html.P("✅ Dash統合機能・レスポンシブUI", style={'margin': '5px 0', 'fontSize': '14px'})
